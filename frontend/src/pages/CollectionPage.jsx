...
export default function CollectionPage() {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(() => {
    const savedValue = localStorage.getItem('cartridge_items_per_page');
  ...
  const dynamicDevelopers = useMemo(() => {
    const developers = userCollection.flatMap(getDeveloperNames);
    return [...new Set(developers)].sort((a, b) => a.localeCompare(b));
  }, [userCollection]);

  const dynamicGenres = useMemo(() => {
    const genres = userCollection.flatMap(getGenreNames);
  ...
  const filteredDevelopers = useMemo(() => {
    ...
    if (!q) return dynamicDevelopers;
    return dynamicDevelopers.filter(d => d.toLowerCase().includes(q));
  }, [dynamicDevelopers, developerSearchQuery]);

  const filteredGenres = useMemo(() => {
    const q = genreSearchQuery.trim().toLowerCase();
    if (!q) return dynamicGenres;
    return dynamicGenres.filter(g => g.toLowerCase().includes(q));
  }, [dynamicGenres, genreSearchQuery]);
  ...
  const processedCollection = useMemo(() => {
    ...
    return [...userCollection]
      .filter((entry) => {
        ...
        const developers = getDeveloperNames(entry).join(' ').toLowerCase();
        const genres = getGenreNames(entry).join(' ').toLowerCase();
        const platform = entry.platformPlayed?.toLowerCase() || '';
      ...
      .filter((entry) => {
        ...
        return entryListIds.some((listId) => filterLists.includes(listId));
      })
      ...
      .filter((entry) => {
        ...
        const meetsMinimum = !yearMin || releaseYear >= Number(yearMin);
        const meetsMaximum = !yearMax || releaseYear <= Number(yearMax);
      ...
      .filter((entry) => {
        ...
        const entryDevelopers = getDeveloperNames(entry);
      ...
      .filter((entry) => {
        ...
        const entryGenres = getGenreNames(entry);

        return entryGenres.some((genre) => filterGenres.includes(genre));
      })
      // Sorting
      .sort((entryA, entryB) => {
        ...
        const developerA = getDeveloperNames(entryA)[0] || '';
        const developerB = getDeveloperNames(entryB)[0] || '';

        const ratingA = Number(entryA.rating) || 0;
        const ratingB = Number(entryB.rating) || 0;

        const dateA = new Date(
          entryA.createdAt || entryA.dateAdded || 0
        ).getTime();

        const dateB = new Date(
          entryB.createdAt || entryB.dateAdded || 0
        ).getTime();
        ...
        switch (collectionSortRule) {
          ...
          case 'dev_desc':
            return developerB.localeCompare(developerA);
          case 'rate_desc':
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(processedCollection.length / effectiveItemsPerPage)
  );
    ...
    >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
          <div>
            ...
            <p style={{ margin: '8px 0 0 0', color: '#6B7280', fontSize: '14px' }}>
              ...
              {processedCollection.length === 1 ? 'title' : 'titles'}
              {userCollection.length !== processedCollection.length && ` out of ${userCollection.length}`}
              .
            </p>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 260px) minmax(0, 1fr)', gap: '32px', alignItems: 'start' }}>
        ...
        <aside style={{ paddingRight: '24px', borderRight: '2px solid #E5E7EB', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div>
            ...
            <select
              ...
              onChange={(event) => {
                setCollectionSortRule(event.target.value);
                setCurrentPage(1);
              }}
              style={{
        ...
        <section style={{ minWidth: 0 }}>
          ...
          {/* Pagination */}
          {!isLoading && processedCollection.length > 0 && (
